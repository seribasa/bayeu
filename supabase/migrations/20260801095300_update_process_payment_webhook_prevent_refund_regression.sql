-- migrate:up
-- Update process_payment_webhook to prevent delayed success webhooks from reversing refunded status
...
BEGIN
    ...
    ELSE
        ...
        IF v_old_status != 'refunded' AND (
            v_old_status NOT IN ('paid', 'settlement') OR p_order_status IN ('paid', 'settlement', 'refunded')
        ) THEN
            UPDATE payments
