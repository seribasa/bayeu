-- migrate:up
-- 1. Deduplicate existing transactions before creating unique constraint
DELETE FROM transactions t1
USING transactions t2
WHERE t1.gateway_transaction_id IS NOT NULL
  AND t1.gateway_transaction_id = t2.gateway_transaction_id
  AND (t1.updated_at < t2.updated_at OR (t1.updated_at = t2.updated_at AND t1.ctid < t2.ctid));

-- 2. Add Unique Constraint on gateway_transaction_id if not exists
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint WHERE conname = 'transactions_gateway_transaction_id_key'
    ) THEN
        ALTER TABLE transactions ADD CONSTRAINT transactions_gateway_transaction_id_key UNIQUE (gateway_transaction_id);
    END IF;
END $$;

-- 2. Update process_payment_webhook to use FOR UPDATE lock and ON CONFLICT upsert
...
BEGIN
    ...
    -- 5. Update Order Status (prevent regressing terminal paid orders)
    IF v_old_status NOT IN ('paid', 'settlement') OR p_order_status IN ('paid', 'settlement') THEN
        UPDATE orders
